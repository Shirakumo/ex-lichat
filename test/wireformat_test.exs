defmodule WireFormatTest do
  ...
  test "printer" do
    assert "0\n\0" == WireFormat.print1(0)
    assert "0.0\n\0" == WireFormat.print1(0.0)
    assert "\"a\"\n\0" == WireFormat.print1("a")
    assert "\"\\\\a\"\n\0" == WireFormat.print1("\\a")
    assert "\"\\\"a\"\n\0" == WireFormat.print1("\"a")
    assert "a\n\0" == WireFormat.print1(Symbol.li("A"))
    assert "\\a\n\0" == WireFormat.print1(Symbol.li("a"))
    assert "a\\a\n\0" == WireFormat.print1(Symbol.li("Aa"))
    assert ":a\n\0" == WireFormat.print1(Symbol.kw("A"))
    assert "b:a\n\0" == WireFormat.print1(Symbol.intern("A", "B"))
    assert "()\n\0" == WireFormat.print1([])
    assert "(0)\n\0" == WireFormat.print1([0])
    assert "(\"a\")\n\0" == WireFormat.print1(["a"])
    assert "(())\n\0" == WireFormat.print1([[]])
    assert "(0 1)\n\0" == WireFormat.print1([0,1])
    assert "(0 1 2)\n\0" == WireFormat.print1([0,1,2])
  end
end
